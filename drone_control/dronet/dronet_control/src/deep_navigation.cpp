...
{
deepNavigation::deepNavigation(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private)
  : nh_(nh), nh_private_(nh_private), name_(nh_private.getNamespace())
{
  ...
  desired_trajectory_sub_ =
      nh_.subscribe("/mavros/trajectory/desired", 1, &deepNavigation::desiredTrajectoryCallback, this);
  local_position_sub_ =
      nh_.subscribe("/mavros/local_position/pose", 1000, &deepNavigation::localPositionCallback, this);
 
  generated_trajectory_pub_ = nh_.advertise<mavros_msgs::Trajectory>("/mavros/trajectory/generated", 10);
...
void deepNavigation::run()
{
  ...
  ros::Rate rate(30.0);
  int state = 0;
  ros::Time last_state_switch = ros::Time::now();
  while (ros::ok())
  {
    ...
    desired_forward_velocity_ =
        (1.0 - alpha_velocity_) * desired_forward_velocity_ + alpha_velocity_ * desired_forward_velocity_m;
    ...
    double x_dir = (desired_trajectory_.point_2.position.x - local_position_.pose.position.x);
    double y_dir = (desired_trajectory_.point_2.position.y - local_position_.pose.position.y);
    double z_dir = (desired_trajectory_.point_2.position.z - local_position_.pose.position.z);
    double mag = sqrt(x_dir * x_dir + y_dir * y_dir + z_dir * z_dir);
    if (mag > 1)
    {  // only make unit if far away from goal
      x_dir /= mag;
      y_dir /= mag;
      z_dir /= mag;
    }
    ...
    if (state == 0 && probability_of_collision_ > 0.1)
    {
      state = 1;
      last_state_switch = ros::Time::now();
    }

    else if (state == 1 && desired_forward_velocity_ > (1 - 0.05) * max_forward_index_ &&
             ros::Time::now() - last_state_switch >= ros::Duration(10.0))
    {
      state = 0;
    }
    ...
    else
    {
      ...
      generated_trajectory_.point_1.position.y = NAN;
      generated_trajectory_.point_1.position.z = NAN;
      generated_trajectory_.point_1.velocity.x = cos(current_yaw_) * desired_forward_velocity_;
      generated_trajectory_.point_1.velocity.y = sin(current_yaw_) * desired_forward_velocity_;
      generated_trajectory_.point_1.velocity.z =
          (desired_trajectory_.point_2.position.z - local_position_.pose.position.z);
      generated_trajectory_.point_1.acceleration_or_force.x = NAN;
      generated_trajectory_.point_1.acceleration_or_force.y = NAN;
      generated_trajectory_.point_1.acceleration_or_force.z = NAN;
      generated_trajectory_.point_1.yaw = NAN;
      generated_trajectory_.point_1.yaw_rate = -desired_angular_velocity_;
...
//    ROS_INFO("desired_forward_velocity_: %.3f , vx: %.3f, vy: %.3f, vz: %.3f, vyaw: %.3f", desired_forward_velocity_,
//             generated_trajectory_.point_1.velocity.x, generated_trajectory_.point_1.velocity.y,
//             generated_trajectory_.point_1.velocity.z, generated_trajectory_.point_1.yaw_rate);
...
void deepNavigation::localPositionCallback(const geometry_msgs::PoseStamped& msg){
  local_position_ = msg;
  current_yaw_ =  tf::getYaw(msg.pose.orientation);
}

void deepNavigation::fillUnusedTrajectoryPoint(mavros_msgs::PositionTarget& point)
{
